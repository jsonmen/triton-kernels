...
# Enable TF32 for PyTorch / cuBLAS
...
def lineartf32_kernel(
    ...
    for k in range(num_pid_k):
        ...
        # Force TF32 Tensor Core execution in Triton
        accumulator = tl.dot(in_, weight, accumulator, input_precision="tf32")
...
def linear_tf32(x: torch.Tensor, weight: torch.Tensor):
...
configs = [
    triton.testing.Benchmark(
        ...
        plot_name="matmul-performance-tf32",
        args={},
    )
]
...
def benchmark(M, N, K, provider):
    ...
    if provider == "triton":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: linear_tf32(inp, weight), quantiles=quantiles
        )
...
if __name__ == "__main__":
    # validate(lineartf32_kernel, dtype=torch.float32)
